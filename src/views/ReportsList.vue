    ...
    <div v-for="reportDate in allDates" :key="reportDate">
      ...
      <div class="flex flex-row flex-wrap">
        ...
        <ReportThumbnailPlaceholder
          ...
          :no-derivatives="
            !reports[reportDate] && !pendingReports.includes(reportDate)
          "
          @refresh="refresh"
          class="w-1/6 m-2 mt-0 mb-8"
        />
...
export default {
  ...
  methods: {
    indexPendingReports() {
      axios
        .get(`${process.env.VUE_APP_API_BASE}/reporting/index-pending-reports`)
        .then(response => {
    ...
    indexReports() {
      ...
      var dateFrom = this.$options.filters.formatDateJson(
      ...
      axios
        ...
        .then(response => {
          ...
          Object.keys(this.reports).forEach(reportDate => {
            this.reports[reportDate][
              this.reports[reportDate].length - 1
            ].status = "ACTIVE";
          });
        })
        .catch(error => {
